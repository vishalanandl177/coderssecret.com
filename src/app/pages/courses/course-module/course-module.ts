        ...
        <main class="flex-1 min-w-0">
          ...
          <div class="flex flex-wrap gap-3 mb-8">
            ...
            @if (labDelivery() === 'inline') {
              <span class="inline-flex items-center gap-2 rounded-full border border-green-500/30 bg-green-500/10 px-5 py-2 text-sm font-semibold text-green-500">
                Inline lab below
              </span>
            } @else {
              <a [href]="'https://github.com/vishalanandl177/' + courseSlug()" target="_blank" rel="noopener noreferrer"
          ...
          @if (m.thinkLikeAnEngineer && m.thinkLikeAnEngineer.length > 0) {
            <div class="rounded-xl border border-purple-500/30 bg-purple-500/5 p-6 mb-8">
              <h2 class="text-lg font-bold text-foreground mb-3">Think Like an Engineer</h2>
              <ul class="space-y-2">
                ...
                @for (lab of m.labs; track lab.title; let i = $index) {
                  <div class="rounded-xl border border-border/60 bg-card p-6">
                    ...
                    @if (lab.repoPath && labDelivery() !== 'inline') {
                      <a [href]="'https://github.com/vishalanandl177/' + courseSlug() + '/tree/main/' + lab.repoPath" target="_blank" rel="noopener noreferrer"
