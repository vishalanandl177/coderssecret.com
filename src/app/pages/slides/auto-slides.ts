...
export class AutoSlidesComponent {
  ...
  private generateSlides(title: string, excerpt: string, html: string, tags: string[], category: string): SlideData[] {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      const plainText = this.htmlToText(sec.html);
      const narration = plainText.slice(0, 500);

      if (codeBlocks.length > 0) {
        const bodyText = this.extractParagraphs(sec.html);
        slides.push({
          type: 'code',
          eyebrow,
          title: sec.heading,
        ...
        for (let c = 1; c < Math.min(codeBlocks.length, 3); c++) {
          slides.push({
            type: 'code',
            eyebrow,
            title: sec.heading + ' (cont.)',
            code: codeBlocks[c].code,
            lang: codeBlocks[c].lang,
            narration: `Here's another code example for ${sec.heading}.`,
          });
        }
        ...
        slides.push({
          ...
          eyebrow,
          title: sec.heading,
          body: this.extractParagraphs(sec.html).slice(0, 200),
    ...
    slides.push({
      ...
      narration: `That's the end of this tutorial. Thanks for watching!`,
    });
