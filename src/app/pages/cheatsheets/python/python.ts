...
export class PythonCheatsheetComponent {
  ...
  sections: Section[] = [
    ...
    },
    {
      title: "Async / Await",
      items: [
        { cmd: "import asyncio", desc: "Import async module" },
        { cmd: "async def fetch():", desc: "Define async function" },
        { cmd: "await coroutine()", desc: "Wait for async result" },
        { cmd: "asyncio.run(main())", desc: "Run async entry point" },
        { cmd: "asyncio.gather(a(), b())", desc: "Run concurrently" },
        { cmd: "asyncio.create_task(coro())", desc: "Schedule in background" },
        { cmd: "async for item in aiter:", desc: "Async iteration" },
        { cmd: "async with aopen('f') as f:", desc: "Async context manager" },
        { cmd: "asyncio.sleep(1)", desc: "Non-blocking sleep" },
        { cmd: "asyncio.Queue()", desc: "Async producer-consumer queue" },
      ],
    },
    {
      title: "Decorators & Metaclasses",
      items: [
        { cmd: "@functools.wraps(fn)", desc: "Preserve wrapped fn metadata" },
        { cmd: "@functools.lru_cache(maxsize=128)", desc: "Memoize function results" },
        { cmd: "@functools.cache", desc: "Unbounded cache (3.9+)" },
        { cmd: "@contextmanager", desc: "Turn generator into context mgr" },
        { cmd: "@classmethod / @staticmethod", desc: "Class-level methods" },
        { cmd: "@abstractmethod", desc: "Force subclass implementation" },
        { cmd: "@overload", desc: "Type-hint multiple signatures" },
        { cmd: "type(name, bases, dict)", desc: "Create class dynamically" },
        { cmd: "class Meta(type):", desc: "Define metaclass" },
        { cmd: "class Cls(metaclass=Meta):", desc: "Use metaclass" },
      ],
    },
    {
      title: "Testing (pytest)",
      items: [
        { cmd: "pip install pytest", desc: "Install pytest" },
        { cmd: "def test_add():", desc: "Test function (prefix test_)" },
        { cmd: "assert result == expected", desc: "Basic assertion" },
        { cmd: "pytest.raises(ValueError)", desc: "Expect exception" },
        { cmd: "@pytest.fixture", desc: "Setup/teardown helper" },
        { cmd: "@pytest.mark.parametrize('a,b', [...])", desc: "Parameterized tests" },
        { cmd: "pytest -v", desc: "Verbose output" },
        { cmd: "pytest -k 'test_name'", desc: "Run specific test" },
        { cmd: "pytest --cov=src", desc: "Coverage report" },
        { cmd: "monkeypatch.setattr(obj, 'attr', val)", desc: "Mock attributes" },
      ],
    },
    {
      title: "Advanced Data Structures",
      items: [
        { cmd: "from heapq import heappush, heappop", desc: "Min-heap (priority queue)" },
        { cmd: "from collections import deque", desc: "Double-ended queue (O(1) both ends)" },
        { cmd: "from collections import OrderedDict", desc: "Insertion-ordered dict" },
        { cmd: "from collections import namedtuple", desc: "Lightweight immutable class" },
        { cmd: "from dataclasses import dataclass, field", desc: "Auto-generated class methods" },
        { cmd: "from enum import Enum, auto", desc: "Enumeration type" },
        { cmd: "from typing import TypeVar, Generic", desc: "Generic types" },
        { cmd: "frozenset({1, 2, 3})", desc: "Immutable set (hashable)" },
        { cmd: "from bisect import insort, bisect_left", desc: "Sorted list operations" },
        { cmd: "from itertools import chain, product, permutations", desc: "Combinatorics" },
      ],
    },
    {
      title: "Performance & Profiling",
      items: [
        { cmd: "python -m cProfile script.py", desc: "Profile entire script" },
        { cmd: "from line_profiler import profile", desc: "Line-by-line profiler" },
        { cmd: "import timeit; timeit.timeit(fn, number=1000)", desc: "Benchmark function" },
        { cmd: "sys.getsizeof(obj)", desc: "Object memory size" },
        { cmd: "__slots__ = ['x', 'y']", desc: "Reduce instance memory 40%" },
        { cmd: "from functools import reduce", desc: "Fold/reduce a sequence" },
        { cmd: "from operator import itemgetter", desc: "Fast key function for sort" },
        { cmd: "from concurrent.futures import ThreadPoolExecutor", desc: "Thread pool" },
        { cmd: "from concurrent.futures import ProcessPoolExecutor", desc: "Process pool" },
        { cmd: "from multiprocessing import Pool", desc: "Parallel map" },
      ],
    },
    {
      title: "Type Hints (Advanced)",
      items: [
        { cmd: "x: int = 10", desc: "Basic type annotation" },
        { cmd: "def fn(x: str) -> bool:", desc: "Function signature" },
        { cmd: "Optional[int]", desc: "int | None" },
        { cmd: "Union[int, str]", desc: "int or str" },
        { cmd: "list[dict[str, Any]]", desc: "Nested generic (3.9+)" },
        { cmd: "Literal['a', 'b']", desc: "Exact value type" },
        { cmd: "TypeAlias = dict[str, list[int]]", desc: "Type alias" },
        { cmd: "Callable[[int], str]", desc: "Function type" },
        { cmd: "Protocol", desc: "Structural subtyping (duck typing)" },
        { cmd: "mypy script.py", desc: "Static type checker" },
      ],
    },
