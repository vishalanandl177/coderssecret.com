...
export const MASTERING_SPIFFE_SPIRE_COURSE: Course = {
    id: 'course-1',
    ...
    modules: [
    {
      ...
      content: `
        ...
        <ul>
          ...
          <li><strong>Does not eliminate policy design complexity:</strong> You still need to design who can talk to whom. SPIFFE makes enforcement possible - it does not design the rules for you.</li>
    ...
    },
    {
      ...
      content: `
        ...
        <p>Both parties share the <strong>same secret key</strong> for encryption and decryption. Fast but has a key distribution problem - how do you securely share the key in the first place?</p>
...
Signature:   (signed by the Intermediate CA's private key)</code></pre>
        ...
        <p>Long-lived certificates are a security risk. If a certificate is compromised, the attacker has access until it expires - which could be years. Short-lived certificates (1 hour or less) limit the blast radius. But short-lived certificates require automatic rotation, which is exactly what SPIRE provides.</p>
      ...
      content: `
        <p>SPIFFE (Secure Production Identity Framework For Everyone) is a set of open standards that define how workloads identify themselves to each other. It does not prescribe an implementation - it defines the <em>what</em>, and projects like SPIRE implement the <em>how</em>.</p>
...
# - Region</code></pre>
        ...
        <p>The Workload API is how workloads request and receive their identities. It is exposed as a <strong>Unix domain socket</strong> - no network communication, no credentials needed. The identity provider (like SPIRE Agent) authenticates the workload by inspecting the calling process (PID, user, container ID) and returns the appropriate SVID.</p>
...
# Applications read these files for mTLS configuration</code></pre>
    ...
    },
    {
      ...
      content: `
        <p>A workload identity system is only useful if applications can actually <em>use</em> the identities. This module covers how to integrate SPIFFE into applications - from zero-code approaches (SPIFFE Helper) to native library integration (go-spiffe, py-spiffe).</p>
...
spire-server bundle set -id spiffe://cluster-b.company.org \\
  -path /path/to/cluster-b-bundle.pem</code></pre>
        ...
        <ol>
          <li><strong>Phase 1 - Deploy SPIRE alongside existing identity:</strong> Run SPIRE in parallel without changing any service. Just get SVIDs flowing.</li>
          <li><strong>Phase 2 - Enable mTLS on one critical path:</strong> Pick one service-to-service connection (e.g., API &rarr; database proxy). Add SPIRE-based mTLS. Keep the old auth as fallback.</li>
          <li><strong>Phase 3 - Expand incrementally:</strong> Service by service, switch from shared secrets to SVID-based authentication. Each switch is independent and reversible.</li>
          <li><strong>Phase 4 - Remove legacy auth:</strong> Once all services use SVIDs, remove the old shared secrets, API keys, and static certificates.</li>
          <li><strong>Phase 5 - Add authorization:</strong> Deploy OPA policies for fine-grained access control on top of the identity layer.</li>
        </ol>
        ...
        <ul>
          ...
          <li><strong>Trust bundle expires?</strong> All SVID verification fails across the trust domain. This is a catastrophic event - monitor CA TTL and rotate well before expiry.</li>
