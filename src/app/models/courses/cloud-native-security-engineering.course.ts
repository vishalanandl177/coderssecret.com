...
- rule: Terminal shell in container
  ...
  priority: WARNING</code></pre>
        ...
        <p>eBPF (extended Berkeley Packet Filter) runs verified programs in the Linux kernel without loading a traditional kernel module. It gives runtime security tools low-level visibility and enforcement hooks, with overhead that depends on the probes, event volume, and userspace processing and should be measured on the target workload.</p>
  ...
  build:
    ...
    steps:
      ...
      content: `
        ...
        <p>For a Django REST Framework application, the <a href="/blog/drf-api-logger-django-rest-framework" class="text-primary underline">maintainer-led DRF API Logger guide</a> demonstrates masked request evidence, correlation, sampled profiling, and retention. Those logs support investigation and detection; they do not replace preventive API controls, Kubernetes audit logs, a SIEM, or distributed tracing.</p>
