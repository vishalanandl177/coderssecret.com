export const CONTENT = `
      <p>Distributed systems algorithms are the load-bearing wall of modern infrastructure. Every Kubernetes cluster, every Kafka pipeline, every Redis Cluster, every Cassandra ring is built on a small set of foundational ideas - consensus, replication, partitioning, gossip, quorum reads - combined in different proportions. Understand those ideas in their pure form, and the &quot;magic&quot; of every distributed system you will ever operate becomes legible.</p>
      ...
      <svg viewBox="0 0 800 360" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="CAP theorem illustration showing the trade-off between consistency, availability, and partition tolerance">
        ...
        <text x="400" y="32" text-anchor="middle" fill="#94a3b8" font-size="14" font-weight="700">CAP THEOREM - PICK TWO DURING A PARTITION</text>
      ...
      <p><strong>The practical lesson:</strong> when you adopt a distributed datastore, do not ask &quot;is it consistent or available?&quot; - ask &quot;during a network partition, would I rather take an outage or serve stale data?&quot; The honest answer depends entirely on the workload. Payments cannot serve stale balances; product catalogue reads can.</p>

      <h2>Consensus - The Hardest Problem in Distributed Systems</h2>

      <p>Consensus is the problem of getting a group of nodes to agree on a single value, even when some of them fail or messages are lost. It is the foundation of every CP system - you need consensus to elect a leader, replicate a write, or coordinate a configuration change.</p>
      ...
      <h3>Paxos - Brilliant but Hard to Implement</h3>

      <p>Leslie Lamport&apos;s Paxos (1989, published 1998) was the first practical consensus algorithm. It guarantees safety (all nodes agree on the same value) under any failure pattern that does not partition more than half the nodes. It works in three phases - <em>prepare</em>, <em>promise</em>, <em>accept</em> - with a designated <em>proposer</em>, a set of <em>acceptors</em>, and <em>learners</em> who eventually receive the chosen value.</p>
      ...
      <ol>
        ...
        <li><strong>Safety:</strong> election restrictions ensure a candidate can only win if its log is at least as up-to-date as a majority of followers - so committed entries never get lost or overwritten.</li>
      </ol>
      ...
      <p>Many systems need a single leader without running full consensus. <strong>Kafka</strong> elects partition leaders via the controller broker, which is itself elected via ZooKeeper (or, in KRaft mode, via an internal Raft group). <strong>Redis Sentinel</strong> elects a single Sentinel as the failover orchestrator using a quorum-based vote. Even a Kubernetes controller manager runs leader election - multiple replicas race to acquire a Lease object in the API server, and only the holder reconciles state.</p>

      <p>The key property all leader-election protocols need is <strong>at most one leader per term</strong>. The Lamport-clock-style monotonic term number is what prevents split-brain - a deposed leader cannot keep accepting writes because every follower will reject any RPC tagged with an old term.</p>
      ...
      <p><strong>Multi-leader replication</strong> (multi-region MySQL with bidirectional replication, CRDT-backed systems) accepts writes at any node. Conflict resolution becomes the central problem - two nodes can accept conflicting writes that need to be merged. Common in geo-distributed systems where local writes matter more than global consistency.</p>
      ...
      <h2>Quorums - The Math of Distributed Reads and Writes</h2>
      ...
      <p>Cassandra exposes these as consistency levels: <code>ONE</code>, <code>QUORUM</code>, <code>LOCAL_QUORUM</code>, <code>EACH_QUORUM</code>, <code>ALL</code>. The <code>LOCAL_QUORUM</code> variant is critical for multi-region deployments - it requires a quorum within the local datacentre but does not wait for cross-region acks, dramatically reducing latency at the cost of cross-region consistency.</p>
      ...
      <h2>Partitioning - Spreading Data Across Nodes</h2>
      ...
      <h2>Gossip Protocols - Membership and Failure Detection at Scale</h2>

      <p>Gossip protocols (also called epidemic protocols) are how large clusters propagate state changes without centralised coordination. Each node periodically picks a few random peers and exchanges state - cluster membership, node liveness, schema versions, partition ownership. Information spreads exponentially: in <code>O(log N)</code> rounds, every node has the latest state.</p>
      ...
      <h2>Vector Clocks and CRDTs - Reasoning About Concurrent Updates</h2>
      ...
      <p>Vector clocks tell you <em>that</em> there is a conflict; <strong>CRDTs</strong> (Conflict-free Replicated Data Types) tell you how to resolve it deterministically. CRDTs are data structures with merge operations that are commutative, associative, and idempotent - meaning replicas can apply updates in any order and still converge to the same state.</p>
      ...
      <h2>Consistency Models - What Your System Promises</h2>
      ...
      <svg viewBox="0 0 800 360" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Consistency models comparison from linearizable to eventual showing trade-offs in latency and availability">
        <rect width="800" height="360" fill="#0f172a" rx="12"/>
        <text x="400" y="32" text-anchor="middle" fill="#94a3b8" font-size="14" font-weight="700">CONSISTENCY MODELS - STRENGTH vs LATENCY</text>
      ...
      <p>A network partition isolates two halves of a cluster, each of which elects a leader and accepts writes. When the partition heals, you have divergent state. Consensus algorithms prevent this by requiring a majority - one side of any partition has minority and cannot elect a leader. Systems without consensus (multi-leader, leaderless) require explicit conflict resolution or can simply lose data.</p>

      <h3>Clock Skew</h3>

      <p>Many algorithms (LWW timestamps, lease-based locking, distributed transactions) assume monotonic, synchronised clocks. Reality: NTP can drift, leap seconds happen, virtual machines can pause. Spanner solved this with TrueTime - a custom hardware-backed clock with bounded uncertainty. CockroachDB uses HLC (Hybrid Logical Clocks) to combine wall-clock time with logical counters. The lesson: <em>do not trust clocks for correctness</em>.</p>
      ...
      <aside class="callout callout-mistake">
        ...
        <p>Running an even-numbered Raft cluster (4 or 6 nodes). It does not give you better fault tolerance than 3 or 5 - you still need a majority - and it raises the cost of every commit. Always odd: 3 for most clusters, 5 for clusters that must tolerate 2 simultaneous failures.</p>
      </aside>
      ...
      <ul>
        <li>Read the <a href="/courses/mastering-spiffe-spire" class="text-primary underline">Mastering SPIFFE &amp; SPIRE</a> course to see how distributed identity systems use these algorithms in production - SPIRE Server uses Raft for HA, federation uses gossip-style trust-bundle exchange, and SVID issuance is a textbook example of leader-routed writes.</li>
