export const CONTENT = `
      <p>Every tutorial on network layers starts with a boring table: "Layer 7 is Application, Layer 6 is Presentation..." and you forget it by next week. This guide is different. We'll learn each layer by <strong>doing</strong> - capturing packets, debugging real problems, and understanding what happens byte-by-byte when you type <code>curl https://api.example.com</code>. The <a href="/blog/websocket-sse-long-polling-realtime-guide">WebSocket, SSE, and long-polling guide</a> applies the transport model to real-time applications, the <a href="/cheatsheets/linux-networking">Linux networking cheatsheet</a> collects the diagnostic commands, and the <a href="/blog/system-design-load-balancer-to-database">browser-to-database system design guide</a> places the layers inside a complete architecture.</p>
