export const CONTENT = `
      <p>Most teams use Prometheus for metrics, but Prometheus was designed as a monitoring tool, not a database. It struggles with long-term storage, high cardinality, and horizontal scaling. <strong>GreptimeDB</strong> is a cloud-native time-series database that speaks PromQL natively - meaning you can replace Prometheus's storage with GreptimeDB and keep your existing Grafana dashboards and alerting rules unchanged. The <a href="/blog/observability-opentelemetry-logs-metrics-traces">OpenTelemetry observability guide</a> explains how applications produce and correlate those signals. If the source is a slow Spark workload, use the <a href="/blog/why-spark-jobs-become-slow-shuffle-skew-partitions-memory">Spark performance guide</a> before blaming the metrics store, and use the <a href="/courses/cloud-native-security-engineering/observability-security-monitoring">security observability module</a> when the dashboard needs audit and detection signals.</p>
