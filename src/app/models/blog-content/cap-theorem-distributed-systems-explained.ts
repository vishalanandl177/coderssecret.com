export const CONTENT = `
      ...
      <div class="flow-diagram">
        ...
        <div class="vs-cards">
          <div class="vs-card" style="border-color:#22c55e">
            <div class="vs-card-header" style="background:#22c55e">C - Consistency</div>
          ...
          <div class="vs-card" style="border-color:#3b82f6">
            <div class="vs-card-header" style="background:#3b82f6">A - Availability</div>
            <div class="vs-card-body">
          ...
          <div class="vs-card" style="border-color:#f97316">
            <div class="vs-card-header" style="background:#f97316">P - Partition Tolerance</div>
            <div class="vs-card-body">
      ...
      <ul>
        <li><strong>MongoDB</strong> (with majority read/write concern) - if the primary node is unreachable, writes are rejected until a new primary is elected.</li>
        <li><strong>Apache ZooKeeper</strong> - used for distributed coordination (leader election, config management). A ZooKeeper cluster that loses quorum stops accepting writes.</li>
        <li><strong>etcd</strong> - the key-value store behind Kubernetes. Uses Raft consensus; a minority partition cannot serve reads or writes.</li>
        <li><strong>Google Spanner</strong> - globally distributed SQL database that uses TrueTime (GPS + atomic clocks) to maintain strong consistency across continents.</li>
      </ul>

      <p><strong>When to choose CP:</strong> Financial transactions, inventory systems, leader election, configuration stores - anywhere stale data leads to incorrect decisions.</p>
      ...
      <ul>
        <li><strong>Apache Cassandra</strong> - every node can serve reads and writes independently. Uses tunable consistency levels (ONE, QUORUM, ALL) to slide between AP and CP behavior.</li>
        <li><strong>Amazon DynamoDB</strong> - designed for "always on" availability. Uses eventual consistency by default; optional strong consistency available per-read.</li>
        <li><strong>CouchDB</strong> - multi-master replication with conflict resolution. Designed for offline-first applications that sync later.</li>
        <li><strong>DNS</strong> - the internet's naming system is a massively distributed AP system. DNS records propagate eventually; stale records are normal and expected.</li>
      </ul>

      <p><strong>When to choose AP:</strong> Shopping carts, social media feeds, analytics dashboards, caching layers, DNS - anywhere a slightly stale response is better than no response.</p>

      <h2>What About CA? (Consistency + Availability)</h2>

      <p>In theory, a <strong>CA system</strong> provides both consistency and availability - but gives up partition tolerance. In practice, <strong>CA systems do not exist in distributed environments</strong> because network partitions are inevitable. You cannot choose to ignore them.</p>
...
# QUORUM = ceil((N+1)/2), so QUORUM + QUORUM > N</code></pre>
      ...
      <p>Even when the network is healthy, maintaining strong consistency requires coordination between nodes (consensus protocols, synchronous replication) - which adds <strong>latency</strong>. Systems that optimize for consistency pay a latency tax. Systems that optimize for latency accept weaker consistency.</p>
      ...
      <ul>
        <li><strong>"CAP means pick 2 out of 3"</strong> - Misleading. Partition tolerance is not optional in any real distributed system. The real choice is between C and A <em>during a partition</em>.</li>
        <li><strong>"MongoDB is CP, so it is always consistent"</strong> - Only if you use majority read/write concern. With default settings, MongoDB can return stale reads from secondaries.</li>
        <li><strong>"Eventual consistency means data is wrong"</strong> - No. It means data might be temporarily stale. In practice, convergence is often sub-second.</li>
        <li><strong>"CAP applies all the time"</strong> - The theorem only constrains behavior <em>during partitions</em>. When the network is healthy, most systems provide both C and A.</li>
        <li><strong>"Single-node databases are immune to CAP"</strong> - True, but they have a single point of failure. The moment you replicate for high availability, CAP applies.</li>
      </ul>
      ...
      <ul>
        ...
        <li><strong>Shopping cart:</strong> AP with conflict resolution. Amazon famously chose AP for its shopping cart (the paper that inspired DynamoDB). Merge conflicts - add both items, let the user resolve.</li>
      ...
      <ul>
        ...
        <li>Most modern databases offer <strong>tunable consistency</strong> - you choose per query.</li>
