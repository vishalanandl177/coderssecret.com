export const CONTENT = `
      ...
      <p>A Kubernetes Operator is a method of packaging, deploying, and managing a Kubernetes application using <strong>custom resources</strong> (CRs) and <strong>custom controllers</strong>. Think of it as a robot SRE that watches your cluster and takes actions to reconcile the actual state with the desired state you've declared. When reconciliation fails, follow the <a href="/blog/kubernetes-debugging-toolkit-kubectl-guide">Kubernetes debugging toolkit</a>; when it succeeds but remains hard to explain, instrument the controller with the <a href="/blog/observability-opentelemetry-logs-metrics-traces">OpenTelemetry observability guide</a>. The <a href="/cheatsheets/kubernetes">Kubernetes cheatsheet</a> keeps the resource-inspection commands within reach.</p>
