...
const GENERATED_BANNER_MARKER = '<!-- Generated by scripts/generate-banners.js -->';
const GENERATED_BANNER_SIGNATURES = [
  '<!-- Decorative circles (seeded from slug) -->',
  '<!-- Icon (right side) -->',
];

function isGeneratedBanner(svg) {
  return svg.includes(GENERATED_BANNER_MARKER)
    || GENERATED_BANNER_SIGNATURES.every(signature => svg.includes(signature));
}

function shouldPreserveExistingBanner(outPath) {
  if (!fs.existsSync(outPath)) {
    return false;
  }

  return !isGeneratedBanner(fs.readFileSync(outPath, 'utf-8'));
}
...
for (const post of posts) {
  ...
  if (shouldPreserveExistingBanner(outPath)) {
    preserved++;
    continue;
  }
...
if (preserved > 0) {
  console.log(`   Preserved ${preserved} custom banner SVGs`);
}
